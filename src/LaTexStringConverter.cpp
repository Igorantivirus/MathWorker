...
namespace mathWorker
{
	...
	std::string LaTexStringConverter::toString(const std::vector<std::string>& params, const std::string& term, SignatureType type, bool needBrackets) const
	{
	...
	std::string LaTexStringConverter::processFunction(const std::vector<std::string>& params, const std::string& term, SignatureType) const
	{
	...
	std::string LaTexStringConverter::processOperation(const std::vector<std::string>& params, const std::string& term, SignatureType) const
	{
		if (params.size() != 2)
	...
	std::string LaTexStringConverter::processSFunction(const std::vector<std::string>& params, const std::string& term, SignatureType) const
	{
		if (params.size() != 2)
			throw ConvertStringException("Two arguments were expected for the term \"" + term + "\".");
		if (term == "root")
	...
	std::string LaTexStringConverter::processUnareOper(const std::vector<std::string>& params, const std::string& term, SignatureType type) const
	{
		if (params.size() != 1)
